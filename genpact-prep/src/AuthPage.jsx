    ...
    try {
      if (mode === "domain") {
        if (email !== "jayarahul696@gmail.com" || password !== "87654321") {
          setError("Invalid Area. Domain credentials do not match.");
          setLoading(false);
          return;
        }
        signInAsDomain();
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: "flex", marginBottom: 32, background: "var(--surface)", borderRadius: 14, padding: 4 }}>
              {["login", "signup", "domain"].map(m => (
                <button key={m} onClick={() => { setMode(m); setError(""); }} type="button" style={{
                  ...
                  fontFamily: "var(--font)", fontSize: 12, fontWeight: 600, transition: "all 0.3s cubic-bezier(0.16,1,0.3,1)",
                  ...
                  {m === "login" ? "Log In" : m === "signup" ? "Sign Up" : "Domain Expert"}
                </button>
              ))}
            </div>

            {/* Google button (hidden for domain) */}
            {mode !== "domain" && (
              <>
            ...
            <form onSubmit={handleSubmit}>
              ...
              >
                {loading ? "Please wait…" : mode === "login" ? "Log In" : mode === "domain" ? "Access Domain Portal" : "Create Account"}
              </button>
            </form>
          </div>
